package com.gyf.os.security.cipher;
import java.io.ByteArrayOutputStream;
import javax.crypto.Cipher;
import org.apache.commons.codec.binary.Base64;
import org.apache.commons.lang.StringUtils;
import com.gyf.os.security.CertInfo;
import com.gyf.os.security.aes.AESSecurityUtil;
public class CipherUtil {
	 ...
	 */
	public static String encryptData(CertInfo publicCert, String data, String signAlgorithms, String charset) throws Exception {
		...
		// 2、用AES法加密数据
		String encryptedData = AESSecurityUtil.encrypt(keyString, data, charset);
		// 3、用RSA加密AES密钥进行保护
		...
		out.close();
		// 受保护的key
		String finalKey = new String(Base64.encodeBase64(encryptedKey), charset);
		return String.format("{\"key\":\"%s\",\"content\":\"%s\"}", finalKey, encryptedData);
	}
	 ...
	 *            私钥
	 * @param encryptedKey
	 *            rsa公钥加密后的byte通过base64编码的字符串
	 * @param encryptedData
	 *            AES加密内容需要先将key进行rsa解密
	 * @param signAlgorithms
	 *            rsa加密算法
	 * @param charset
	 *            字符集
	 ...
	 * @see
	 */
	public static String decryptData(CertInfo privateCert, String encryptedKey, String encryptedData, String signAlgorithms, String charset) throws Exception {
		if (StringUtils.isBlank(signAlgorithms)) {
			signAlgorithms = PKCS1Padding;
		}
		if (StringUtils.isBlank(charset)) {
		...
		// 2、解密内容
		String decryptData = AESSecurityUtil.decrypt(keyString, encryptedData, charset);
		// 3、返回
		return decryptData;
	}
}
